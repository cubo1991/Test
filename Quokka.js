...
const paddockManagers = [
  ...
  */
  ...
  function listPaddockManagersByName() {
    let managersMap = paddockManagers.slice().sort(function (a, b) {
      return a.name.localeCompare(b.name);
    }).map((paddockManager) => {
      return {
        name: paddockManager.name,
        taxNumber: paddockManager.taxNumber
      };
    });
    let managersReturned = managersMap.map((e) => e.taxNumber);
    return managersReturned;
  };
  ...
  function sortPaddockTypeByTotalArea() {
    let paddocksNuevos = paddockType

    for (let i = 0; i < paddockType.length; i++) {
        paddocksNuevos[i].totalCultivos = 0  
       
        for (let j = 0; j < paddocks.length; j++){
            if(paddocksNuevos[i].id === paddocks[j].paddockTypeId){
            paddocksNuevos[i].totalCultivos += paddocks[j].area}
        }   
    ...
    return paddocksNuevos.sort(function (a,b) {return -a.totalCultivos - -b.totalCultivos})
  }
  ...
  function sortFarmManagerByAdminArea() {
    let paddocksNuevos = paddockManagers

    for (let i = 0; i < paddockManagers.length; i++) {
        paddocksNuevos[i].totalCultivos = 0  
       
        for (let j = 0; j < paddocks.length; j++){
            if(paddocksNuevos[i].id === paddocks[j].paddockManagerId){
            paddocksNuevos[i].totalCultivos += paddocks[j].area}
        }   
  ...
  function farmManagerNames() {
    ...
    for (let i = 0; i < paddockManagers.length; i++) {
      for (let j = 0; j < paddockType.length; j++) {
        paddocks.forEach((e) => {
          if (e.farmId === paddockType[j].id && e.paddockManagerId === paddockManagers[i].id) {
            nuevoPaddocks.push({ [paddockType[j].name]: paddockManagers[i].name });
          }
        });
      }
    }

    if (nuevoPaddocks.length > 0) {      
      return nuevoPaddocks.sort(function(a, b) {
        var valueA = Object.values(a)[0]; 
        var valueB = Object.values(b)[0]; 
  ...
  function biggestAvocadoFarms() {
    let farmsTotal = farms
    let contador = 0
   for (let i = 0; i < paddocks.length; i++) {  
    for (let j = 0; j < paddockType.length; j++) {
       if(paddockType[j].name === 'PALTOS' ){
        if((paddocks[i].paddockTypeId === paddockType[j].id ) && paddocks[i].area > 20000){
            contador += 1
            for (let z = 0; z < farms.length; z++) {
        
                if(paddocks[i].farmId === farms[z].id){
                
                farmsTotal[z].metrosTotal = 0
                console.log(farmsTotal[z])
                farmsTotal[z].metrosTotal =+ paddocks[i].area 
                console.log(farmsTotal[z])}
            }} if (contador === 0) { return "No hay campos que tengan mas de 2 hectáreas en Paltos"} 
       }
    }
   }
   return farmsTotal
  ...
  function biggestCherriesManagers() {
    let administradoresTotal = paddockManagers
    let administradoresFiltrados;
        for (let i = 0; i < paddocks.length; i++) {  
     for (let j = 0; j < paddockType.length; j++) {
  
            
      if (paddockType[j].name === 'CEREZAS' && (paddockType[j].id === paddocks[i].paddockTypeId) && paddocks[i].area > 10000){   
                
      for(let e = 0; e <  paddockManagers.length; e++) {
        
        if(paddockManagers[e].id === paddocks[i].paddockManagerId){
          console.log(paddockManagers[e].id)
         
        
          administradoresTotal[e].metrosTotal = 0
         
          administradoresTotal[e].metrosTotal =+ paddocks[i].area 
       
          return  administradoresFiltrados = administradoresTotal.filter(admin => admin.metrosTotal > 10000).sort()
        }
  ...
  function farmManagerPaddocks() {
    ...
    for (let paddock of paddocks) {
      let farm = farms.find(f => f.id === paddock.farmId);
      let manager = paddockManagers.find(m => m.id === paddock.paddockManagerId);
      
      if (!result[manager.name]) {
        result[manager.name] = [];
      }
  
      if (!result[manager.name].includes(farm.name)) {
        result[manager.name].push(farm.name);
      }
    }
  
    // ordenar los nombres de las farms alfabéticamente
    for (let farms of Object.values(result)) {
      farms.sort();
    }
  ...
  function paddocksManagers() {
    ...
    for (let i = 0; i < paddocks.length; i++) {
      for (let j = 0; j < paddockType.length; j++) {
        for (let z = 0; z < paddockManagers.length; z++) {
          if(paddocks[i].paddockTypeId === paddockType[j].id){
            const nombre = paddockType[j].name;
            const año = paddocks[i].harvestYear;
            const idManager = paddocks[i].paddockManagerId;
            const nameManager = paddockManagers[idManager-1].name;
            const key =  `${nombre}-${año}`
            const value =`${idManager} ${nameManager}`
            
            if (!result[key]) {
              result[key] = {};
            }
            result[key][idManager] = [value]
          }
  ...
  function newManagerRanking() {
    let newManager = {id: 7, taxNumber: '78684748', name: 'DAVID LOPEZ MATHEZ' }
    let newPaddock = {paddockManagerId: 7, farmId: 1, paddockTypeId: 4, harvestYear: 2017, area: 900} 
    paddockManagers.push(newManager)
    paddocks.push(newPaddock)
  paddockManagers
  let ranking = sortFarmManagerByAdminArea()
  console.log(ranking)
  const found = ranking.findIndex(e => e.id === 7)
  return found + 1  
